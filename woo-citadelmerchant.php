	...
	*/
	...
	public function http_request($url, $method='GET', $data=array(), $flags='')
	{
		...
		try {
			...
			$logger->error( $msg, array( 'source' => 'citadel_merchant' ) );
			throw $e;
		}
	}
		...
		foreach ($items as $item)
		{
			        ...
			        != $item_total)
			{
				...
				while ($this->precise($item_price * $item->get_quantity(), $prec)
				        > $item_total)
				{
					$item_price -= (1/pow(10,$prec));
				}
				$item_price = $this->precise($item_price, $prec);
			}
			...
			$count_total = $this->precise($count_total, $prec);
		}
